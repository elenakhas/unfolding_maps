...
public class ParseFeed {
	...
	public static List<PointFeature> parseEarthquake(PApplet p, String fileName) {
		...
		for (int i = 0; i < itemXML.length; i++) {
				...
				String date = getStringVal(itemXML[i], "updated");
				
				int year = Integer.parseInt(date.substring(0,4));
				point.putProperty("year", year);
				int month = Integer.parseInt(date.substring(5, 7));
				point.putProperty("month", month);
				int day = Integer.parseInt(date.substring(8, 10));
				point.putProperty("day", day);
				String time = date.substring(11, 19).replace(":", "");
				
				long tim = Long.parseLong(time);
				point.putProperty("time", tim);
	...
	public static List<ShapeFeature> parseRoutes(PApplet p, String fileName) {
		...
		for(String row : rows) {
			...
			if(!columns[3].equals("\\N") && !columns[5].equals("\\N")){
				...
				route.putProperty("from", columns[2]);
				route.putProperty("to", columns[4]);
